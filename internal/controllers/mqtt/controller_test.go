...
func TestNewDefaults(t *testing.T) {
	svc := newDefaultSvc()
	c, err := New(nil, svc, Config{DeviceID: "room101"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewValidation(t *testing.T) {
	...
	if _, err := New(nil, svc, Config{}); err == nil {
		t.Fatal("expected error when DeviceID missing")
	}

	if _, err := New(nil, svc, Config{DeviceID: "x", QoS: 2}); err == nil {
...
func TestTopicJoin(t *testing.T) {
	svc := newDefaultSvc()
	c, err := New(nil, svc, Config{DeviceID: "room101", BaseTopic: "thermocktat/room101/"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestOnMessage_Enabled(t *testing.T) {
	svc := newDefaultSvc()
	c, _ := New(nil, svc, Config{DeviceID: "room101"})
	fc := &fakeClient{}
	c.client = fc
	c.onMessage(nil, fakeMessage{
...
func TestPublishSnapshot_PublishesJSON(t *testing.T) {
	svc := newDefaultSvc()
	c, _ := New(nil, svc, Config{DeviceID: "room101", QoS: 1, RetainSnapshot: true})
